...
interface PluginAPI {
  ...
  flatten(layers: ShapeGroup[]): (ShapeGroup | Path)[] | null; // 将shapeGroup转换为path
  outline(layers: (Path | ShapeGroup)[]): (Path | ShapeGroup)[] | null; // 将路径轮廓化
...
interface Layer {
  ...
  readonly parent: Page | Frame | Group | ShapeGroup | null;
...
interface SymbolMaster extends Frame {
  ...
  createSymbolInstance(): SymbolInstance | null;
}
